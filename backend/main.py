...
async def update_config(config: dict):
    ...
    global LANE_CONFIG
    if "fast_limit" in config: LANE_CONFIG["fast_limit"] = config["fast_limit"]
    if "normal_limit" in config: LANE_CONFIG["normal_limit"] = config["normal_limit"]
    return {"status": "updated", "config": LANE_CONFIG}

# --- PRIORITY MAPPING ---
def map_priority(intent):
    # CRITICAL: Money related, complaints
    if intent in ['payment_issue', 'get_refund', 'track_refund', 'complaint', 'check_cancellation_fee', 'fraud_report']:
        return 1
...
async def chat_endpoint(request: ChatRequest):
    ...
    lane_limit = LANE_CONFIG["fast_limit"] if is_fast_lane else LANE_CONFIG["normal_limit"]
    ...
    async with state_lock:
        if lane_state[lane_key] >= lane_limit:
            logger.warning(f"⛔ [Ticket {ticket_id}] {lane_name} Full! ({lane_state[lane_key]}/{lane_limit})")
            raise HTTPException(status_code=status.HTTP_429_TOO_MANY_REQUESTS, detail=f"{lane_name} Full")

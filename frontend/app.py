...
def extract_pii_type(text):
    ...
    # 1. Strong Matches
    if re.search(r'\b(?:\d[ -]*?){13,19}\b', text): return "Credit Card"
    if re.search(r'[\w\.-]+@[\w\.-]+\.\w+', text): return "Email Address"
    if re.search(r'(?:\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}', text): return "Phone Number"
    ...
    if re.search(r'\b\d{7,11}\b', text):
        # Context Check
        if any(kw in text_lower for kw in ['card', 'visa', 'master', 'credit', 'debit']):
...
if col1.button("▶ START"): st.session_state.stress_active = True
if col2.button("⏹ STOP"): st.session_state.stress_active = False
...
try:
    ...
    if res.status_code == 200:
        ...
        if fast_total == 0: fast_total = 1
        if norm_total == 0: norm_total = 1
        ...
        if st.session_state.stress_active:
             ...
             est_fast = st.session_state.get('sc_crit', 0) + st.session_state.get('sc_high', 0)
...
with col_chat:
    ...
    if 'last_meta' in st.session_state:
        ...
        if 'explainability' in meta:
            ...
            prio_map = {
                "Critical": ["payment_issue", "get_refund", "track_refund", "complaint", "check_cancellation_fee", "fraud_report"],
                "High": ["cancel_order", "change_order", "change_shipping_address", "place_order", "track_order", "delivery_options", "delivery_period"],
                "Normal": ["chit_chat", "greeting", "account_info", "newsletter"] # Catch-all
            }
            ...
            for intent, prob in exp.items():
                ...
                for p_label, intents in prio_map.items():
                    if intent in intents:
                        ...
                        found = True
                        break
                if not found: prio_probs["Normal"] += prob # Fallback
            ...
            with tab_prio:
                # Priority Chart
                pdf = pd.DataFrame(list(prio_probs.items()), columns=['Priority', 'Probability'])
                st.bar_chart(pdf, x='Priority', y='Probability', color="#6610f2", height=200)
                
            with tab_intent:
                # Optimized Intent Chart (Top 7)
                exp_df = pd.DataFrame(list(exp.items()), columns=['Intent', 'Probability'])
                exp_df = exp_df.sort_values(by='Probability', ascending=False).head(7)
                st.bar_chart(exp_df, x='Intent', y='Probability', color="#007BFF", height=250)
        ...
        if "messages" in st.session_state:
            for msg in st.session_state.messages:
                if msg['role'] == 'user':
                     ...
                     badge = msg.get('badge', '')
                     st.markdown(f"<div class='chat-bot'>{badge}<br>{msg['content']}</div>", unsafe_allow_html=True)
    ...
    if prompt:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                st.session_state.messages.append({"role": "assistant", "content": data['message'], "badge": badge})
                st.rerun()
...
with col_monitor:
    # Tabs for Clean UI
    tab_traffic, tab_analytics, tab_security = st.tabs(["🚦 Live Traffic", "📊 Analytics", "🛡️ Security"])
    ...
    with tab_traffic:
        ...
        if st.session_state.stress_history:
            ...
            st.area_chart(hist_df[line_cols], color=["#DC3545", "#FD7E14", "#28A745", "#5bc0de"], height=180)

    with tab_analytics:
        st.subheader("🔥 Live Chaos Metrics")
        if st.session_state.stress_active:
            m1, m2, m3, m4 = st.columns(4)
            total_ok = st.session_state.stress_stats["Critical"] + st.session_state.stress_stats["High"] + st.session_state.stress_stats["Normal"]
        ...
        if st.session_state.stress_history:
             ...
             st.bar_chart(chart_df[cols], color=["#DC3545", "#FD7E14", "#28A745", "#5bc0de", "#007BFF"], height=250)
        else:
    ...
    with tab_security:
        ...
        if st.session_state.stress_traffic_log:
            ...
            pii_df = df_sec[df_sec["PII"] == True].copy()
            if not pii_df.empty:
...
if st.session_state.stress_active:
    ...
    for _ in range(count_crit): payloads.append({"text": random.choice(CRITICAL_SENTS)})
    for _ in range(count_high): payloads.append({"text": random.choice(HIGH_SENTS)})
    for _ in range(count_norm): payloads.append({"text": random.choice(NORMAL_SENTS)})
    for _ in range(count_pii): payloads.append({"text": random.choice(PII_SENTS)})
    random.shuffle(payloads)
    
    # Update max_workers to match load
    total_reqs = len(payloads)
    if total_reqs == 0: total_reqs = 1 # avoid 0 workers
    ...
    def send_req(payload):
        try:
            ...
            if res.status_code == 200:
                ...
                return 200, d.get('label', 'NORMAL'), d.get('pii_detected', False), d.get('intent', '-'), d.get('explainability', {})
            return res.status_code, 'BLOCKED', False, 'throttled', {}
        except: return 0, 'ERR', False, 'error', {}
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=total_reqs) as executor:
        ...
        for f in concurrent.futures.as_completed(f_map):
            ...
            if status == 200:
                ...
                if pii:
                    current_pii += 1
                    st.session_state.stress_stats["PII"] = st.session_state.stress_stats.get("PII", 0) + 1
            else:
